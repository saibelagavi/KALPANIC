...
function getClient(): Cerebras {
  if (!_client) {
    if (!process.env.CEREBRAS_API_KEY) throw new Error('CEREBRAS_API_KEY is not configured');
    _client = new Cerebras({ apiKey: process.env.CEREBRAS_API_KEY });
  }
  return _client;
}
...
export async function generatePlaybookContent(input: GeneratePlaybookInput): Promise<AIBlock[]> {
  ...
  let raw = '';
  for await (const chunk of stream as AsyncIterable<{ choices: { delta?: { content?: string | null } }[] }>) {
    raw += chunk.choices[0]?.delta?.content ?? '';
  }

  // Robustly extract JSON — handle markdown fences or extra text
  const jsonMatch = raw.match(/\{[\s\S]*\}/);
  if (!jsonMatch) throw new Error('Cerebras returned unparseable response');

  let parsed: { blocks: AIBlock[] };
  try {
    parsed = JSON.parse(jsonMatch[0]);
  } catch {
    throw new Error('Cerebras JSON parse failed');
  }

  if (!Array.isArray(parsed.blocks) || parsed.blocks.length === 0) {
    throw new Error('Cerebras returned no blocks');
  }
